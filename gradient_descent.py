'''
    1. 경사 하강법 (gradient descent)
        ...
        2) 그래디언트(기울기)는 접선의 기울기로 이해해도 됨
            ...
            - 손실함수를 가중치로 미분한 값이 양수 ---> 가중치를 감소시킴
            - 손실함수를 가중치로 미분한 값이 음수 ---> 가중치를 증가시킴
        3) 손실함수(loss, cost function)을 최소화 하기 위해
            함수(f)의 계수(coefficient) 값을 찾기 위해 사용되는 최적화 알고리즘
                ...
                - 경사 하강법의 목표는 cost 함수를 최소화하기 위해 계수 값을 찾는 것임
                - 계수의 경사도(gradient)를 계산한 후, 경사도가 가리키는 방향으로 계수를 업데이트하는 것으로 작동
...
# 손실함수 정의
loss_func = lambda x: (x - 3) ** 2 + 10     # ** 2 는 제곱이라는 뜻
